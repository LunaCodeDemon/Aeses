...
async def generate_whois_embed(member: hikari.Member) -> hikari.Embed:
    ...
    embed = hikari.Embed(
        title=f"Whois of {member.display_name}", color=member.accent_color
    )
    ...
    embed.add_field(
        name="Creation", value=member.created_at.strftime(TIMEFORMAT), inline=False
    )
    if member.joined_at:
        embed.add_field(
            name="Joined", value=member.joined_at.strftime(TIMEFORMAT), inline=False
        )
...
async def info_command(
    ctx: tanjun.abc.Context, bot: hikari.GatewayBot = tanjun.inject()
):
...
async def invite_command(
    ...
    link = (
        ...
        "&permissions=2281712656&scope=bot%20applications.commands"
    )
    embed = hikari.Embed(
        title="Invite",
...
@tanjun.with_member_slash_option(
    "member", "The member to get info about.", default=None
)
...
async def update_bot_statistics(bot: hikari.GatewayBot = tanjun.inject()):
    ...
    if bot.application:
        bots_gg.update_statistics(
            bot.application.id, len(bot.cache.get_guilds_view())
        )
