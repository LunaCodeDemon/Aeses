...
@tanjun.with_int_slash_option(
    "seconds", "The number of seconds from now to set the reminder for.", min_value=1
)
@tanjun.as_slash_command(
    "reminder", "(Instable) Set a reminder that will send you a message in a given time."
)
...
@tanjun.with_channel_slash_option(
    "channel", "The channel to set as the log channel.", default=None
)
...
async def log_add_command(
    ctx: tanjun.abc.Context, logtype: str, channel: hikari.InteractionChannel | None
):
...
async def log_list_command(ctx: tanjun.abc.Context):
    ...
    for logchannel in channels:
        embed.add_field(
            name=logchannel.logtype.name, value=f"<#{logchannel.channel_id}>"
        )
...
async def reminder_update(bot: hikari.GatewayBot = tanjun.inject()):
    ...
    reminders_to_process = [
        r
        for r in reminders
        if numpy.datetime64(r.trigger_at) <= numpy.datetime64(timestamp)
    ]
    ...
    for remind in reminders_to_process:
        try:
            ...
            if not target:
                logging.warning("Reminder target channel %s not found.", remind.channel_id)
                continue
            ...
            logging.warning("Could not send reminder %s: %s", remind.id, e)
        except hikari.HikariError as e:
            logging.error(
                "An unexpected Hikari error occurred while processing reminder %s: %s",
                remind.id,
                e,
            )
...
async def on_member_join(
    event: hikari.MemberCreateEvent, bot: hikari.GatewayBot = tanjun.inject()
):
...
async def on_member_leave(
    event: hikari.MemberDeleteEvent, bot: hikari.GatewayBot = tanjun.inject()
):
    """React when a member leaves or gets kicked"""
    try:
        async for entry in bot.rest.fetch_audit_log(
            guild=event.guild_id, action_type=hikari.AuditLogEventType.MEMBER_KICK
        ).limit(1):
            if entry.target == event.user:
                log_channel_data = sqldata.get_logchannel(
                    event.guild_id, sqldata.LogType.MODERATION
                )
                if log_channel_data:
                    channel = await bot.rest.fetch_channel(
                        log_channel_data[0].channel_id
                    )
                    embed = await create_moderation_embed(
                        event.user, "kick", entry.reason or "No reason given"
                    )
                    await channel.send(embed=embed)
                return
    except hikari.ForbiddenError:
        logging.warning(
            "Missing permissions to fetch audit log in guild %s", event.guild_id
        )
...
async def on_member_ban(
    event: hikari.BanCreateEvent, bot: hikari.GatewayBot = tanjun.inject()
):
    """React on ban."""
