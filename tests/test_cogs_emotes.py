...
@pytest.fixture
def mock_config():
    """Fixture for a mocked config dictionary."""
    config_data = {
        "emotes": {
            "hug": {
        ...
        "emote_links": {
            "hug": ["http://example.com/hug.gif"]
        }
    }
    with mock.patch.dict(emotes_cog.config, config_data, clear=True):
        with mock.patch.dict(emotes_cog.emote_links, config_data['emote_links'], clear=True):
            yield

def test_generate_emoji_embed(mock_config):
...
@pytest.mark.anyio
async def test_create_emote_command(mock_ctx, mock_config):
