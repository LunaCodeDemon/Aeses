...
@mock.patch.object(settings_cog, "generate_filtertype_listing", side_effect=lambda x: str(x))
async def test_filterconf_list(mock_generate_list, mock_get_config, mock_ctx):
...
async def test_filterconf_toggle(mock_insert_config, mock_get_config, mock_ctx):
    ...
    mock_insert_config.assert_called_once_with(12345, sqldata.FilterType.EMOJI_NAME, True)
